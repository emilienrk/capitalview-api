...
- **Libellé proche, définition exacte.** Mots = `type_rules.telling_words` : suites de lettres, une suite contenant un chiffre écartée entière (référence de virement qui change chaque mois — mesuré sur le dump : le salaire VILMORIN tombait à 0,27 de similarité avec `label_words`, 0 fausse fusion ajoutée) ; mots informatifs = mots − `patterns.label_common(compte, sens)` : mots présents dans > `COMMON_WORD_SHARE` des **libellés distincts** (signatures) du côté, et dans ≥ `COMMON_WORD_MIN_COUNT` d'entre eux. Compté par libellé et non par opération (`patterns.common`) : l'employeur d'un salaire revient chaque mois et passerait pour courant, la règle n'atteindrait jamais la référence du mois suivant ; score = Jaccard (∩ / ∪) des mots informatifs de la ligne et de la règle ; proche si score ≥ `transfer_decisions.SIMILARITY_THRESHOLD`. Plusieurs règles proches : meilleur score, puis la plus récente. Aucun mot informatif → jamais proche.
