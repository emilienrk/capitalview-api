...
#
# A flat bps threshold cannot be the check, though: a trader who genuinely times
# a volatile stock well can post hundreds of basis points of "slippage" every
# month, and that is a real signal, not a data defect. The check instead asks
# whether the paid price is one the asset was actually quoted at that month —
# a venue mismatch fails that test regardless of amplitude, and real skill
# passes it regardless of amplitude too.
...
def test_a_normal_series_is_plausible():
    ...
    assert result.out_of_range_share == Decimal("0")


def test_a_venue_mismatch_is_flagged_as_implausible():
    """Every order 12% off the stored close: not extreme, impossible — the
    month never quoted anywhere near that price."""
    result = analyse_execution(_orders_at("112"), _flat_month(), rng=rng(0))

    assert result.out_of_range_share == Decimal("1")
    assert result.is_plausible is False


def test_one_bad_order_does_not_condemn_the_series():
    """A minority out of range can be a fat-fingered entry, not a venue
    mismatch — only a majority gates the whole block."""
...
def _volatile_v_shaped_month(month_index: int, days: int = 20):
    """A month that dips then partly recovers — deterministic, no randomness.

    The trough is always the lowest quoted price of the month, so a buyer who
    times it perfectly pays exactly a price the asset really traded at.
    """
    base = Decimal("100") + Decimal(month_index) * Decimal("2")
    trough = days // 2
    quotes = {}
    for i in range(days):
        day = date(2024 + month_index // 12, month_index % 12 + 1, 1) + timedelta(days=i)
        distance = abs(i - trough)
        # Down to -18% at the trough, back up toward the month's start.
        price = base * (Decimal("1") - Decimal("0.18") * Decimal(max(0, trough - distance)) / Decimal(trough))
        quotes[day] = price
    return quotes, min(quotes.items(), key=lambda kv: kv[1])


def test_a_genuine_timing_skill_on_a_volatile_stock_stays_plausible():
    """The case the flat threshold used to swallow.

    Twenty-four months, buying exactly the monthly low of a stock swinging 18%
    within the month. The "slippage" is large and highly significant — and it
    is real: every price paid is a price the stock actually traded at.
    """
    matrix = {"VOLATILE": {}}
    orders = []
    for m in range(24):
        month_quotes_, (low_day, low_price) = _volatile_v_shaped_month(m)
        matrix["VOLATILE"].update(month_quotes_)
        orders.append(_Tx("BUY", "VOLATILE", low_day, price=str(low_price)))

    result = analyse_execution(orders, matrix, rng=rng(0))

    assert result.is_plausible is True
    assert result.out_of_range_share == Decimal("0")
    # The signal is real and large — this is what the guard must not suppress.
    assert result.weighted_slippage_bps < Decimal("-500")
