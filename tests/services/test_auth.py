...
import jwt # This is PyJWT based on auth.py imports "import jwt"
import nacl.pwhash
...
def test_create_and_decode_access_token():
    ...
    assert decoded_exp["exp"] - expected_exp.timestamp() < 10 # within 10 seconds
    ...
    bad_payload = {"sub": "user", "type": "refresh"} # wrong type
...
def test_refresh_token_lifecycle(session: Session):
    # Ensure user exists first (foreign key constraint might be active)
...
def test_verify_refresh_token_expired(session: Session):
    ...
    # Create manually to set past expiration
    rt = RefreshToken(
        user_uuid=user_uuid,
        token=token_str,
...
def test_get_master_key():
    ...
    # 3. Preference (Cookie > Header? No, logic is "or", so first non-None)
...
def test_create_refresh_token_db_custom_expiry(session: Session):
    ...
    assert expires_at > datetime.now(timezone.utc)
    # Check it's roughly 1 hour from now
