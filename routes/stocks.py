...
def get_all_history(
    ...
    return get_all_stock_accounts_history(session, current_user.uuid, master_key, include_current)
...
def get_account_history_route(
    ...
    if not get_stock_account(session, account_id, current_user.uuid, master_key):
        raise HTTPException(status_code=404, detail="Account not found")
    return get_stock_account_history(session, account_id, master_key, include_current)
