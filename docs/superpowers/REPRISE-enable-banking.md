...
| **R18** | **La réconciliation a quatre issues** (`estimated` ajoutée par R23) : `reconciled` / `gap` / `not_reconcilable` / `estimated`. Un compte carte a un écart **permanent et normal** ; une alerte permanente sur un fonctionnement normal détruit le signal sur *tous* les comptes | un état d'affichage à retirer |
...
| **R23** | **`ITAV` accepté en dernier recours, et la courbe est alors `estimated`.** Revolut ne publie qu'un `InterimAvailable` — aucun `CLBD` sur aucun compte — donc le refus strict de §F rendait la banque définitivement insynchronisable. `ITAV` = solde comptable moins ce qui est bloqué : la synchro rajoute les lignes *pending* qu'elle voit, et Revolut n'en partage aucune, d'où le verdict `estimated` plutôt qu'un `gap` affiché à chaque paiement carte qui se comptabilise. `XPCD` reste exclu : rien ne permet de le corriger | une courbe décalée du montant des paiements en attente, et l'écart mesuré qu'il faudra relire sur données réelles pour trancher |
| **R24** | **Le premier seeding part du serveur, dès le rattachement, et deux synchros d'un même utilisateur ne se chevauchent plus.** Revolut ne sert l'historique complet que 5 minutes après l'autorisation, 90 jours ensuite ; laissé à l'appel post-rendu du front, un écran de choix resté ouvert perdait la fenêtre pour de bon. `seed_after_linking` tourne en `BackgroundTasks` ; le verrou advisory bloquant `wait_for_lock` (par `user_bidx`) fait attendre l'appel concurrent du front, qui relit ensuite des liens déjà capés. En complément, `history_served_from_enc` mesure jusqu'où le seeding a remonté (élargi, jamais rétréci) et le front l'affiche comme une limite plutôt que de proposer un nouvel essai que la banque refuserait pareil | un rattachement qui déclenche un appel bancaire en arrière-plan ; le verrou est un no-op hors Postgres, donc non exercé par les tests SQLite |
| **R25** | **Un échec consomme la synchro du jour, comme un succès.** Le plafond ne lisait que `last_synced_at`, que seul un succès fait avancer : un compte en erreur restait dû, et comme le front synchronise après chaque rendu, chaque visite de la page Banque rappelait la banque. `last_sync_attempt_at` compte toute tentative, `last_sync_error_enc` garde la raison (auparavant perdue au rechargement). Réessayer avant le lendemain est un geste explicite : `POST /banking/accounts/{uuid}/retry-sync`, refusé sur un compte qui n'a pas échoué ; une reconnexion rend aussi la tentative | une panne bancaire passagère attend le lendemain ou un clic sur « Réessayer » |
