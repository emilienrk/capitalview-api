...
def test_get_market_price_expired_cache(session: Session, mock_market_manager):
    ...
    # Handle naive datetime from SQLite for test assertion
...
def test_get_market_price_fetch_fail_expired_cache(session: Session, mock_market_manager):
    """Test fetch failure with expired cache (should return stale data)."""
    symbol = "STALE-USD"
    price = Decimal("100.0")
    ...
    assert result == price # Returns stale price
    mock_market_manager.get_info.assert_called_once()

# ---- Tests for get_market_info (Tuple return) ----
...
def test_get_market_price_cache_hit_naive_datetime(session: Session, mock_market_manager):
    ...
    entry = MarketPrice(
        ...
        current_price=price,
        currency="USD",
        last_updated=now_naive # Helper in services/market.py should fix this to UTC
    )
    session.add(entry)
    session.commit()
