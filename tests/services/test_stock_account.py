...
def test_update_stock_account(session: Session, master_key: str):
    ...
    # Re-fetch via service to check decryption
    fetched = get_stock_account(session, created.id, user_uuid, master_key)
...
def test_delete_stock_account(session: Session, master_key: str):
    ...
    # 3. Delete account WITH transactions (Cover cascading delete logic)
    ...
    # We need to create a transaction. We can import create_stock_transaction or just insert one manually.
    # Let's use the service if possible, or manual to avoid circular dep if any (tests should be fine).
